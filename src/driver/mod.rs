...
impl X86_64Driver {
    fn compile_function(&self, function: Function, code: &mut String) {
        ...
        for operation in function.body {
            self.compile_operation(operation, code);
        }
    ...
    fn compile_operation(&self, operation: Operation, code: &mut String) {
        match operation {
            Operation::Store { variable_index, value } => {
                code.push_str(&format!(
                    "    mov dword ptr [rbp-{}], {}\n",
                    variable_index * value.size(),
    ...
    fn compile_value(&self, value: Value) -> String {
        match value {
            ...
            Value::VariableReference(variable_index) => format!("dword ptr [rbp-{}]", variable_index * value.size()),
        }
    }
}
